...
<Route
    path="/document"
    element={isAuthenticated ? <Document /> : <Navigate to="/login" replace />}
/>
</Routes>
...
- Functional React component with state management and API integration.
- Features:
...
|----------------|-------------------------------------|---------|
| response       | PaginatedResponse<Document> \| null | Stores fetched document list with pagination info |
| page           | number                               | Tracks current page number |
| pageSize       | number                               | Number of rows per page |
| loading        | boolean                              | Indicates if data is being fetched |
| departments    | any[]                                | Stores department options for filtering |
| categories     | any[]                                | Stores category options for filtering |
| departmentId   | string                               | Selected department filter |
| categoryId     | string                               | Selected category filter |
| search         | string                               | User-entered search keyword |
...
```ts
useEffect(() => { ... }, [page, pageSize, departmentId, categoryId, search])
```

- Fetches documents from the API whenever any filter or pagination changes.
- API Call:

```ts
GET / documents;
Params: (page, per_page, department_id, category_id, search);
```

2. Fetch Filters (Departments & Categories)
...
- Fetches filter options from:
- /departments – Department list.
- /document-categories – Category list.
- Populates departments and categories state arrays.
